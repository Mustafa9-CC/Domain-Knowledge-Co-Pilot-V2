...
# ---------------------------------------------------------------------------
# Detect database type and create the engine with appropriate arguments.
...
# Only register the listener for SQLite; PostgreSQL enforces FKs natively.
# ---------------------------------------------------------------------------
if _is_sqlite:
...
def _run_migrations(bind):
    ...
    The migration logic uses SQLite-specific statements (PRAGMA,
    INSERT OR IGNORE, table-rebuild pattern) and is skipped entirely
    for PostgreSQL where create_all() handles fresh table creation.
    """
    if not _is_sqlite:
        logger.info("Skipping SQLite-specific migrations (PostgreSQL detected)")
        return
